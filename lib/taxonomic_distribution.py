...
def taxonomic_distribution(total_reads, absent, present, agora2_level_set, df, level, levels):
    ...
    df_present_species = df.loc[df['Species'] != pd.NA]
    ...
    df_absent_species, df_present_species = agora_checking.agora_checking(df_present_species, agora2_level_set)
    
    return df_present_species, df_absent_species, absent/total_reads